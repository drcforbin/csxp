...
extern const std::string_view arbitrary_clj = R"-(
    (defn argcount ([] 0) ([x] 1) ([x y] 2) ([x y & more]
      (+ (argcount x y) (count more))))
    (argcount) (argcount 1 2 3 4 5)
    (defn make-adder [x] (let [y x] (fn [z] (+ y z))))
    (def add2 (make-adder 2))
    (+ (add2 4) (add2 4) (add2 4) (add2 4) (add2 4) (add2 4))
    (* (add2 4) (- 9 4) (/ 397845 4) (or (prn 4) 0)
        (add2 4) (add2 (* (+ (inc (- 4 1)) 3) 8)))
    (let [my-vector [1 2 3 4] my-map {:fred "ethel"}
      my-list (list 4 3 2 1)]
      (list (conj my-vector 5) (assoc my-map :ricky "lucy")
      (conj my-list 5)
      ; some kinda comment
      my-vector my-map my-list))
    {:keyw1 val1 :kw3 :kw4 :a 9 9 :a :hdh nil true false false true}
    (defmacro if-valid
      "Handle validation more concisely"
      [to-validate validations errors-name & then-else]
      `(let [~errors-name (validate ~to-validate ~validations)]
      (if (empty? ~errors-name) ~@then-else)))
    (defmacro yes-no->boolean1 [val] `(let [b (= ~val "yes")] b))
    (defmacro yes-no->boolean2 [val] `(let [b# (= ~val "yes")] b#))
    `(let [~errors-name? (validate ~to-validate ~validations)]
      (if (empty? ~errors-name) ~@then-else))
    ["a" "b" "cde" "fgh" "ijk" "\o233" "\u273" 0x12327 0237 0x8274]
    (defmacro yes-no->boolean1 [val] `(let [b (= ~val "yes")] b))
    (defmacro yes-no->boolean2 [val] `(let [b# (= ~val "yes")] b#))
)-"sv;

extern const std::string_view arbitrary_long_clj = R"-(
